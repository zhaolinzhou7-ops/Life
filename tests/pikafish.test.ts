/**
 * 专业引擎（Pikafish 皮卡鱼）的接线层：坐标、分数、info 行、着法表。
 * 引擎本体在浏览器里跑（界面测试覆盖），这里锁死的是两边对接的口径——
 * 口径错一点，教练就会把"红方被杀"读成"红方杀人"。
 */
import { describe, expect, it } from 'vitest';
import { PRO_SCALE, linesToMoveScores, moveToUci, parseInfo, toOurScore, uciToMove } from '../src/xiangqi/pikafish';
...
describe('坐标', () => {
  it('a0 是红方左下角，第 9 行是黑方底线（皮卡鱼的记法，行号从 0 开始）', () => {
    // 红方右马 (7,9) 跳到 (6,7)：马二进三 = h0g2
    const m = { fx: 7, fy: 9, tx: 6, ty: 7 };
    expect(moveToUci(m)).toBe('h0g2');
    expect(moveToText(initialBoard(), m)).toBe('马二进三');
    // 黑方左车 (0,0) → a9
    expect(moveToUci({ fx: 0, fy: 0, tx: 0, ty: 1 })).toBe('a9a8');
  });
  ...
  it('乱七八糟的输入返回 null', () => {
    for (const s of ['', 'z1a2', 'a10a1', 'a1a', '(none)']) expect(uciToMove(s)).toBeNull();
  });
});
...
describe('分数', () => {
  it('厘兵分按比例换到我们的口径（车≈1000）', () => {
    // 实测皮卡鱼一个车约 560
    expect(toOurScore(560, null).score).toBe(Math.round(560 * PRO_SCALE));
    expect(Math.abs(toOurScore(560, null).score - 1000)).toBeLessThan(20);
  });
...
describe('info 行', () => {
  it('解析深度、第几条、分数、主变', () => {
    const l = parseInfo('info depth 19 seldepth 28 multipv 2 score cp -806 nodes 527723 nps 175790 time 3002 pv a0a2 f4e2 f0e1')!;
    expect(l.depth).toBe(19);
    expect(l.multipv).toBe(2);
    expect(l.score).toBe(Math.round(-806 * PRO_SCALE));
    expect(l.pv).toEqual(['a0a2', 'f4e2', 'f0e1']);
...
describe('着法表：前几名精确，其余只知道上限', () => {
  ...
  const lines = [
    parseInfo('info depth 12 multipv 1 score cp 20 pv h0g2 h9g7')!,
    parseInfo('info depth 12 multipv 2 score cp 15 pv g0e2 h9g7')!,
  ];
  ...
  it('引擎给出不合法的着法（数据错位）就丢掉，不让它混进教练的判断', () => {
    const bad = [parseInfo('info depth 3 multipv 1 score cp 0 pv a0a8')!];
