 ...
 */
export const DEPTHS = [12, 13, 14];
export const TIME_FOR = (d: number) => (d >= 14 ? 8000 : 3000);
 ...
 *   3. **一次下出来的结果不算数**：12/13/14 三个深度下出同一个结果才收，
 *      不一致说明这个局面就在胜和边界上，没有稳定答案，不该拿去当教材。
 */
export function verdict(b: Board, you: Color): {
  ...
  resetEngine();
  const runs = DEPTHS.map((d) => playOut(b, you, d, TIME_FOR(d)));
