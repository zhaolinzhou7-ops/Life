 ...
 * 题库的加载与选题。
 *
 * 题目由 tools/gen-puzzles.ts 离线生成，入库前过 tools/audit-data.ts 的体检
 * （局面能在真实对局里出现、答案合法、杀法步数准确、主变走得通）。
 *
 * 这里原来写的是"答案唯一"——**那句话是假的**。全量核对之后发现
 * 398 道杀法题里有 30 道要么步数标错、要么同样步数还有别的杀法，
 * 学生走出另一手同样快的杀棋会被判错。所以现在多了 `also` 字段：
 * 一样好的着法一律算对。
 *
...
export interface Puzzle {
   ...
   * 和 answer 一样好的其它着法。
   *
   * 杀法题里同样步数的杀棋往往不止一手，战术题里也常有两手等价。
   * 判对错只比 answer 一个字符串的话，学生走对了会被判错——对
   * 有点水平的人来说这是最伤的一种 bug，所以这些着法必须一起算对。
   */
  also?: string[];
  /**
